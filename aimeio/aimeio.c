...
extern void DataManageInit(void);
extern unsigned char DataManage_GetCard(unsigned char* id);
extern void DataManage_InputRGB(unsigned char* rgb);
        ...
        size_t luid_size)
{
    ...
    unsigned char ID[10];
    if (DataManage_GetCard(ID) == 1)
    {
        memcpy(luid, ID, luid_size);
        return S_OK;
    }
